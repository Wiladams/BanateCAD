...
function TextBox:CalculateTextOrigin()
	...
	local xCenter = self.Frame.Origin[1] + self.Frame.Dimension[1] / 2;
	local yCenter = self.Frame.Origin[2] + self.Frame.Dimension[2] / 2;
    ...
    elseif self.HAlignment == Alignment.Right then
		self.TextOrigin[1] = self.Frame.Origin[1] + self.Frame.Dimension[1] - stringSize[1];
	end
    ...
    elseif self.VAlignment == Alignment.Bottom then
		self.TextOrigin[2] = self.Frame.Origin[2] + self.Frame.Dimension[2] - stringSize[2]/2;
	end
end
...
function TextBox:RenderBackground(graphPort)
	...
	elseif self.BackColor[4] ~= 0 then
		...
		graphPort:DrawRect(self.Frame.Origin[1], self.Frame.Origin[2],
			self.Frame.Dimension[1], self.Frame.Dimension[2]);
	end
end
...
--]]
	...
	return vec3(self.Frame.Dimension[1], self.Frame.Dimension[2],0);
end
