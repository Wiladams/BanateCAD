...
function GraphicArrow:_init(params)
	...
	self.ArrowDepth = params.ArrowDepth or self.Frame.Dimension[2] * 0.1;
	...
	local right = x + self.Frame.Dimension[1];
	local bottom = y + self.Frame.Dimension[2];

    self.Vertices = {
		vec3(x, y + self.DropMargin),
		vec3(right - self.ArrowDepth, y + self.DropMargin),
		vec3(right - self.ArrowDepth, y),
		vec3(right, y + (self.Frame.Dimension[2] / 2)),
...
function GraphicArrow:GetClientFrame()
	local rect = Rectangle(
		...
		self.Frame.Dimension[1] - self.ArrowDepth - 4,
		self.Frame.Dimension[2] - (self.DropMargin * 2) - 4);
